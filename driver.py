...
class driver:
    ...
    def run(self, G1, G2, G3, G4,G5, G6,G7, G8,G9, G10, v, save_path):
        ...
        while True:  # 迭代更新标签
            ...
            # _host_label_dict=[]
            # n_list=[x for x in range(0,10)]
            t=[x for x in range(0,10)]
            for i in range(0,10):
                map = [G1, G2, G3, G4, G5, G6, G7, G8, G9, G10]
                n_list=[intersect_ids[0],intersect_ids[1],intersect_ids[2],intersect_ids[3],intersect_ids[4],intersect_ids[5],intersect_ids[6],intersect_ids[7],intersect_ids[8],intersect_ids[9]]
                _host_label_dict = _host.get_label_dict(map[i])
                map1=map[i]
                ...
                for j in range(0,9):
                    v_label[j]=_guest.get_label_dict(map[j])
            # # print("_host_label_dict===",_host_label_dict)
            # # print("_guest_label_dict1====",_guest_label_dict1)
            #
            # # 分别获取host,guest各自相交顶点的标签数字典
            # for i in range(0,10):
                _host_intersect_dict = _host.get_intersect_dict(_host_label_dict, intersect_ids1)
                _guest_intersect_dict =[x for x in range(0,9)]
                for j in range(0,9):
                    _guest_intersect_dict[j]=_guest.get_intersect_dict(v_label[j], n_list[j])
                # _guest_intersect_dict2 = _guest.get_intersect_dict(v_label[1], intersect_ids[2])
                # _guest_intersect_dict3 = _guest.get_intersect_dict(v_label[2], intersect_ids[3])
                # _guest_intersect_dict4 = _guest.get_intersect_dict(v_label[3], intersect_ids[4])
                # _guest_intersect_dict5 = _guest.get_intersect_dict(v_label[5], intersect_ids[5])
                # _guest_intersect_dict6 = _guest.get_intersect_dict(v_label[0], intersect_ids[6])
                # _guest_intersect_dict7 = _guest.get_intersect_dict(v_label[0], intersect_ids[7])
                # _guest_intersect_dict8 = _guest.get_intersect_dict(v_label[0], intersect_ids[8])
                # _guest_intersect_dict9 = _guest.get_intersect_dict(v_label[0], intersect_ids[9])
            # print("_host_label_dict===", _host_intersect_dict)
            # print("_guest_label_dict1====", _guest_intersect_dict1)

            # 加密相交顶点的标签数并发送到coordinator
                _guest_encrypt_time=[x for x in range(0,9)]
                _host_intersect_dict, _host_encrypt_time = _host.send_intersect_dict(_host_intersect_dict)
                for j in range(0,9):
                    _guest_intersect_dict[j], _guest_encrypt_time[j] = _guest.send_intersect_dict(_guest_intersect_dict[j])
            ...
            # intersect_dict =[_host_intersect_dict,_guest_intersect_dict[0],_guest_intersect_dict[1],_guest_intersect_dict[2]\
            #                   ,_guest_intersect_dict[3],_guest_intersect_dict[4],_guest_intersect_dict[5],_guest_intersect_dict[6],\
            #                  _guest_intersect_dict[7],_guest_intersect_dict[8]]
            #     intersect_dict=[x for x in range(0,10)]
            #     intersect_dict[i]=_host_intersect_dict
                for j in range(0,9):
                    # if i!=j:
                    _host_intersect_dict=_coordinator.send_intersect_dict(_host_intersect_dict,_guest_intersect_dict[j])
                # if i==0:
            ...
            # print("intersect_dict[0]====",intersect_dict[0])
                ...
                _host.update_intersect_nodes(map1, _host_intersect_dict, v)
                _host_remain_up_time, _host_remain_up_num = \
                _host.update_remain_nodes(map1, _host_label_dict, intersect_ids1, v)
                ...
                t[i]=map1
            # _guest_decrypt_time=[x for x in range(0,9)]
            # _guest_intersect_up_time=[x for x in range(0,9)]
            # _guest_intersect_up_num=[x for x in range(0,9)]
            # for j in range(0,9):
            #     _guest_decrypt_time[j], _guest_intersect_up_time[j], _guest_intersect_up_num[j] = _guest.update_intersect_nodes(map[j], intersect_dict[j], v)
            ...
            G9 = t[8]
            G10 = t[9]
            communities = _coordinator.get_communities(t)
